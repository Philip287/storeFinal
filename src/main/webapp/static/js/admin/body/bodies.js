$(document).ready( function () {
    ...
    let table = $('#bodies_table').DataTable( {
        ...
        columns: [
            ...
            },
            {
                data: null,
                render: function (row) {
                    return '<a href="/controller?command=go_to_edit_body_page&id=' + row.entityId + '" type="button" class="btn btn-outline-primary me-1">'
                        + editData + '</a>'
                        + '<a href="/controller?command=delete_body&id=' + row.entityId + '" type="button" class="btn btn-outline-primary me-1">'
                        + deleteData + '</a>'
                }
            },
        ],
    ...
    function onDataTableInitComplete(table) {
        ...
        $('#createButton').click(function () {
            window.location.href = "/controller?command=go_to_create_body_page";
        });
